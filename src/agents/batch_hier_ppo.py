...
from environments.batch_acquire_env import AcquireEnv
from src.policies.batch_hier_ppo import *
...
class Agent(object):
    ...
    def _setup(self, env):
        ...
        num_afa_actions = 2 ** env.num_measurable_features
    ...
    def rollout(self, env, rand_afa, rand_tsk):
        ...
        while not done:
            ...
            if rand_afa:
                ...
                afa_data = Batch(
                    full=state, obs=obs, act=afa_res.act[0], rew=reward, done=terminate, 
            ...
            # tsk
            tsk_data = Batch(full=state, obs=obs)
            if rand_tsk:

...
class POExVAE(nn.Module):
    def __init__(
        ...
        self.obs_dim = obs_dim = obs_space.shape[0]
        obs_embed_dim = obs_dim * 2 # concat mask
        ...
        self.decoder = ConDecoder(decoder_input_dim, config.peq_decode_dims, obs_dim, 
            config.num_heads, config.num_inds, config.use_ln)
        
    def _obs_embed(self, state, mask):
        return torch.cat([state, mask], dim=2)
    ...
    def forward(self, state, mask, action):
        ...
        decoder_outputs = self._decode(decoder_inputs, posterior_samp, num_posterior_samp)

        # log_likelihood
        state = state.unsqueeze(0).expand(num_posterior_samp, -1, -1, -1) #[N,B,T,d]
        state = state.contiguous().view([-1]+list(state.shape[2:])) #[N*B,T,d]
        log_likel = - torch.sum(torch.square(decoder_outputs - state).view([num_posterior_samp,-1]+list(state.shape[1:])), dim=(2,3)).mean(dim=0)
    ...
    def impute(self, state, mask, action, num_samples):
        ...
        # decoder
        decoder_inputs = torch.cat([prior_inputs, peq_embed], dim=-1)
        decoder_samp = self._decode(decoder_inputs, prior_samp, num_samples)
    ...
    def accuracy(self, state, mask, action, num_samples):
        ...
        error = torch.sum(torch.square(samp - state) * (1-mask))
    ...
    def reward(self, state, mask, action, num_samples):
        ...
        real = state[:,-1] #[B,d]
        m = mask[:,-1] #[B,d]
        error = torch.sum(torch.square(samp - real) * (1-m), dim=(0,2))

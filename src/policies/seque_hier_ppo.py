...
class PolicyBuilder:
    ...
    def build_afa_policy(self):
        if self.env.observation_type == "Categorical" and self.env.action_type == "Discrete":
            ...
            num_afa_actions = self.env.num_measurable_features + 1 # termination action
            num_tsk_actions = self.env.action_space.n

            actor = CatPObsActor(self.config, num_embeddings, num_afa_actions, True)
        ...
        if self.env.observation_type == "Continuous" and self.env.action_type == "Discrete":
            ...
            actor = ConPObsActor(self.config, obs_dim, num_afa_actions, True)
    ...
    def build_tsk_policy(self):
        if self.env.observation_type == "Categorical" and self.env.action_type == "Discrete":
            ...
            actor = CatPObsActor(self.config, num_embeddings, num_tsk_actions, False)
            critic = CatPObsCritic(self.config, num_embeddings)
            return ActorCritic(actor, critic)
        ...
        if self.env.observation_type == "Continuous" and self.env.action_type == "Discrete":
            ...
            actor = ConPObsActor(self.config, obs_dim, num_tsk_actions, False)
            critic = ConPObsCritic(self.config, obs_dim)
            return ActorCritic(actor, critic)